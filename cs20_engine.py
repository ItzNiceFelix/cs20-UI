...
def _init_lang(lang: str):
    ...
    _LANG_TRANSCRIPT_MAP = {
        ...
        "jp": ["ja", "ja-JP"],
        "kr": ["ko", "ko-KR"],
        "in": ["hi", "hi-IN", "te", "te-IN"],  # auto-detect: Hindi atau Telugu
    }
    TRANSCRIPT_LANGS = _LANG_TRANSCRIPT_MAP.get(lang, ["id", "en", "id-ID"])

    if lang == "in":
        # Gabung tier Hindi + Telugu jadi satu set matching aktif,
        # supaya satu engine bisa nangkep kedua aksara tanpa perlu tahu
        # dulu video-nya bahasa/aksara apa.
        _hi_tiers = _ALL_KEYWORD_TIERS["in"]
        _te_tiers = _ALL_KEYWORD_TIERS["te"]
        merged = {}
        for tier_name in _hi_tiers:
            merged[tier_name] = {
                "bobot": _hi_tiers[tier_name]["bobot"],
                "patterns": (
                    _hi_tiers[tier_name]["patterns"]
                    + _te_tiers.get(tier_name, {}).get("patterns", [])
                )
            }
...
def get_video_ids(channel: str, limit: int, content_type: str) -> list:
    ...
    if content_type == "live":
        ...
        # Gabung, deduplikasi (by video id), batasi limit
        seen, combined = set(), []
        for vid, title in ids_live + ids_video:
            if vid not in seen:
                seen.add(vid)
                combined.append((vid, title))
...
def _fetch_ids(url: str, limit: int, extra_args: list) -> list:
    """Helper: jalankan yt-dlp flat-playlist dan return list (id, title).
    Judul diambil bareng ID dalam satu call yang sama, tidak nambah request."""
    cmd = [
        "yt-dlp",
        "--flat-playlist",
        "--print", "%(id)s|||%(title)s",
...
# Map video_id -> title, diisi sekali per channel dari get_video_ids()
# (title didapat gratis dari call yt-dlp yang sudah ada, tanpa request tambahan)
_VIDEO_TITLES: dict = {}

# Saat True, engine dipanggil dari UI (Streamlit) — output jadi JSON per baris
# ke stdout (prefix "CS20JSON:") alih-alih Rich Live UI, supaya gampang
# di-parse dan di-stream ke web UI secara live.
JSON_MODE: bool = False

def _emit_json(event_type: str, payload: dict):
    """Print satu baris JSON event untuk dikonsumsi UI (Streamlit)."""
...
def analyze_video(video_id: str, channel: str) -> dict:
    ...
    base_result = {
        ...
        "channel": channel,
        "transcript_lang": "",   # diisi setelah fetch berhasil (hi/te/id/dst)
    ...
    # ── FETCH TRANSKRIP ──────────────────────────────────────────────
    try:
        segments, detected_lang = _fetch_transcript(video_id)
        base_result["status"] = "ok"
        base_result["transcript_lang"] = detected_lang
...
def _run_pantau_json(channel, video_ids, worker_fn, checkpoint_info: dict, webhook_url: str = ""):
    """Versi run_display_pantau untuk UI: tetap paralel (ThreadPoolExecutor),
    tapi output-nya JSON event per baris ke stdout, bukan Rich Live UI."""
    global _stats

    results            = []
    consecutive_errors = 0

    _emit_json("channel_start", {"channel": channel, "total": len(video_ids)})
    ...
    with ThreadPoolExecutor(max_workers=_stats.get("_jobs", 4)) as pool:
        ...
        for future in as_completed(futures):
            ...
            # Progress event — tiap video selesai (match atau tidak)
            _emit_json("progress", {
                "channel": channel,
                "done": done,
                "total": len(video_ids),
            ...
            # Match event — cuma kalau video ini punya hit (grouped, sesuai
            # struktur result yang sudah ada: 1 result = 1 video + list hits)
            if result.get("hits"):
                _emit_json("match", {
                    "channel":         channel,
            ...
            if consecutive_errors >= 10:
                ...
                _kirim_laporan_darurat("rate limit darurat")
                _emit_json("rate_limit_stop", {"channel": channel, "done": done})
                pool.shutdown(wait=False, cancel_futures=True)
                break

    _emit_json("channel_done", {"channel": channel, "done": len(results)})
    return results, consecutive_errors
...
def run_display_pantau(channel, video_ids, worker_fn, checkpoint_info: dict, webhook_url: str = ""):
    ...
    if JSON_MODE:
        return _run_pantau_json(channel, video_ids, worker_fn, checkpoint_info, webhook_url)
...
def process_channel(args):
    ...
    if args.retry_blocked_log and os.path.exists(args.retry_blocked_log):
        ...
        video_pairs = get_video_ids(channel, limit, content_type)
        video_ids = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json-events", action="store_true",
                         help="Output JSON event per baris ke stdout untuk dikonsumsi UI (Streamlit)")
