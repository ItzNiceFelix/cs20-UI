...
def validate_channel(channel: str, content_type: str = "all") -> dict | None:
    ...
    # PENTING: field "playlist_count" itu TIDAK reliable untuk channel tab
    # URL (/videos, /streams) kalau dibarengi --playlist-items 0 — YouTube
    # channel tab diperlakukan beda dari playlist biasa, dan yt-dlp sering
    # nggak sempat menghitung playlist_count-nya (output kosong meski rc=0,
    # sehingga kelihatan seolah "0 video" padahal aslinya ada banyak).
    # Solusi: ambil ID video beneran (dibatasi 200 biar cepat) dan hitung
    # dari situ — teknik yang sama seperti get_video_ids(), sudah terbukti
    # reliable.
    def _approx_count(tab: str, cap: int = 200) -> tuple[int, bool]:
        """Return (count, is_capped). is_capped=True kalau >= cap (approx, bukan pasti)."""
        rc_c, out_c, _ = run_ytdlp([
        ...
        if rc_c != 0:
            return 0, False
        ids = [l for l in out_c.splitlines() if l.strip()]
        return len(ids), len(ids) >= cap

    video_count = 0
    video_capped = False
    if check_videos:
        video_count, video_capped = _approx_count("videos")

    live_count = 0
    live_capped = False
    if check_live:
        live_count, live_capped = _approx_count("streams")

    return {
        "username": ch,
        "name": name or ch,
        "video_count": video_count,
        "video_capped": video_capped,
            ...
            with st.spinner("Validasi channel..."):
                for ch in channels:
                    ...
                    if info and info["valid"]:
                        vid_display = (f"{info['video_count']:,}{'+' if info.get('video_capped') else ''}"
                                       if content_type_code in ("all", "video") else "–")
                        live_display = (f"{info['live_count']:,}{'+' if info.get('live_capped') else ''}"
                                        if content_type_code in ("all", "live") else "–")
